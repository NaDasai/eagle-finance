...
-   **Token Deployer**: A smart contract that allows deploying MRC-20 tokens.
...
-   **Synchronize Reserves:** Syncs the reserves of the pool with the current balances of the tokens. Only the owner (the owner of the registry contract) can call this function.
-   **Get the LP Balance of a user:** Get the balance of LP tokens of a user in the contract.
-   **Get the local reserve of A:** Returns the reserve of token A in the pool.
-   **Get the local reserve of B:** Returns the reserve of token B in the pool.
-   **Get the Price of A in terms of B:** Returns the price of token A in terms of token B (reserveB/reserveA).

**Key Features:**

-   **MRC-20 Support:** Designed to work primarily with MRC-20 tokens and also with wrapped MAS (WMAS).
-   **Fee Structure:** Implements a swap fee that's split between liquidity providers and the protocol.
-   **Native MAS Support:** Allows swapping directly with native MAS when it is wrapped to WMAS.
